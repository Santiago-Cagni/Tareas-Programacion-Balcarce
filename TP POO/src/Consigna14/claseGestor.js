...
class GestorLibros {
    ...
    modificarLibro(libro, nuevosDatos) {
        const libroModificado = this.libros.find(lib => lib.ISBN === libro.ISBN);
        if (libroModificado) {
            if (nuevosDatos.titulo)
                libroModificado.titulo = nuevosDatos.titulo;
            if (nuevosDatos.autor)
                libroModificado.autor = nuevosDatos.autor;
            if (nuevosDatos.numeroPag)
                libroModificado.numeroPag = nuevosDatos.numeroPag;
